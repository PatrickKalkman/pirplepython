...
Let's return to the music example from assignment one. Pick 3 of the attributes
 you listed. For our example we're going to say "Genre", "Artist" and "Year".
Create a new Python file and create 3 functions with the same name as those
attributes. So in this example we'd have one function named "genre" another
named "artist" and another called "year".
...
For extra credit, see if you can figure out how to use booleans, and add an
extra function that returns a boolean value instead of a String or Number.
Hint: make sure to capitalize the first letter in the words True or False
when you use them. We'll cover booleans more in the lecture on "if" statements
...
def bio():
    return "Christopher Alvin Stapleton is an American singer-songwriter, " + \
        "guitarist, and record producer. Born in Lexington, Kentucky," + \
        " and grew up in Staffordsville, Kentucky, moving to Nashville, " + \
        "Tennessee, in 2001 to pursue a career in music writing songs. " + \
        " Subsequently, Stapleton signed a contract with Sea Gayle to " + \
